...
public class SQSMessageConsumerPrefetch implements Runnable, PrefetchManager {
	...
	/**
	 * Counter on how many messages have been explicitly requested. TODO:
	 * Consider renaming this class and several other variables now that this
	 * logic factors in message requests as well as prefetching.
	 */
	...
	protected void setMessageListener(MessageListener messageListener) {
		...
		synchronized (stateLock) {
			...
			// TODO: This may overfetch if setMessageListener is being called
			// multiple
			// times, as the session callback scheduler may already have entries
			// for this consumer.
	 ...
	 * <P>
	 * After consumer is closed, all the messages inside internal buffer will be
	...
	protected List<Message> getMessages(int prefetchBatchSize) throws InterruptedException {
		...
		ReceiveMessageRequest receiveMessageRequest = new ReceiveMessageRequest(queueUrl)
				.withMaxNumberOfMessages(prefetchBatchSize).withAttributeNames(ALL).withMessageAttributeNames(ALL)
				.withWaitTimeSeconds(WAIT_TIME_SECONDS);
		// if the receive request is for FIFO queue, provide a unique receive
		// request attempt it, so that
	...
	protected javax.jms.Message convertToJMSMessage(Message message) throws JMSException {
		...
		MessageAttributeValue replyToQueueNameAttribute = message.getMessageAttributes()
				.get(SQSMessage.JMS_SQS_REPLY_TO_QUEUE_NAME);
		MessageAttributeValue replyToQueueUrlAttribute = message.getMessageAttributes()
				.get(SQSMessage.JMS_SQS_REPLY_TO_QUEUE_URL);
	...
	public void messageListenerReady() {
		synchronized (stateLock) {
			// messagesRequested may still be more than zero if there were
			// pending receive()
	...
	/**
	 * Helper that notifies PrefetchThread that message is dispatched and
	 * AutoAcknowledge
	 */
